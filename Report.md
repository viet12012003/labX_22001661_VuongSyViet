...
- **Giai đoạn luật-concatenative (trước deep learning)**  
  Nhiều hệ thống TTS cổ điển dựa trên các luật ngữ âm, hệ thống từ điển phát âm và nối các đơn vị tiếng nói đã được ghi âm sẵn (diphone, triphone, unit selection). Xem trong sách của Taylor [Taylor, 2009].
...
- **Thời kỳ neural TTS (Level 2)**  
  DeepMind công bố WaveNet [Oord et al., 2016], Google/DeepMind ra Tacotron, Tacotron 2 [Shen et al., 2018]; Microsoft, Baidu, ... đề xuất Deep Voice, FastSpeech, FastPitch.  
  Gần đây xuất hiện mô hình end-to-end như VITS [Kim et al., 2021] kết hợp text → mel + vocoder trong một mạng duy nhất.

- **Thời kỳ few-shot / zero-shot (Level 3)**  
  Các công trình như YourTTS [Casanova et al., 2022], VALL-E [Wang et al., 2023], Voicebox [Le et al., 2023] dùng kiến trúc encoder–decoder lớn, mã hoá giọng thành embedding và sinh tiếng nói mới từ vài giây ví dụ.

- **Xu hướng mới: Expressive & controllable TTS, watermarking & chống deepfake**  
  Nhiều survey (ví dụ Anusuya & Katti, 2024; IJCRT 2025 [IJCRT, 2025]) nhấn mạnh việc thêm điều khiển cảm xúc, phong cách, độ nói nhanh–chậm, cũng như nghiên cứu watermark hoá đầu ra để tránh lạm dụng deepfake.
...
- **Thương hiệu, doanh nghiệp** muốn có giọng riêng (giọng tổng đài, giọng trợ lý ảo, MC ảo) với chất lượng cao.
...
- **Voicebox** [Le et al., 2023] và các hệ thống tương tự:  
...
- **Doanh nghiệp muốn giọng thương hiệu riêng, chất lượng cao, không đổi quá thường xuyên** → **Level 2**.
...
- **Watermark tín hiệu audio**: chèn mẫu tín hiệu ở miền tần số/thời gian sao cho bền vững với nén, truyền qua mạng nhưng không làm giảm chất lượng cảm nhận.  
  Tham khảo các nghiên cứu watermark audio tổng quát trong sách Cox et al., 2008.

- **Watermark ở mức mô hình / token**: với các hệ thống dựa trên token (codec token, mel token), có thể áp dụng ý tưởng tương tự **watermark cho LLM** (như trong công trình của Kirchenbauer et al., 2023) để điều chỉnh phân phối xác suất sinh token theo một pattern bí mật.
...
- Shen, J., Pang, R., Weiss, R. J., Schuster, M., Jaitly, N., Yang, Z., ... & Wu, Y. (2018). Natural TTS synthesis by conditioning WaveNet on mel spectrogram predictions. *ICASSP*.
...
- Casanova, E., Weber, J., Shulby, C., Gölge, E., et al. (2022). YourTTS: Towards zero-shot multi-speaker TTS and zero-shot voice conversion for everyone. *arXiv:2112.02418*.
- Wang, C., Chen, S., Wu, Y., Zhang, Z., et al. (2023). Neural codec language models are zero-shot text-to-speech synthesizers. (VALL-E). *arXiv:2301.02111*.
- Le, P., et al. (2023). Voicebox: Text-guided multilingual universal speech generation at scale. *Meta AI report*.
- Cox, I. J., Miller, M. L., Bloom, J. A., Fridrich, J., & Kalker, T. (2008). *Digital Watermarking and Steganography*. Morgan Kaufmann.
- IJCRT. (2025). A Comprehensive Review on Text-To-Speech (TTS). *International Journal of Creative Research Thoughts*, IJCRT2507281.